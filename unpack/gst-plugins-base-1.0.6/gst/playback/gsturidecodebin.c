...
//2013-03-27 gstreamer upgrade start
  ...
  gint drm_type;
  guint thumbnail_mode;         /* thumbnail property for thumbnail mode */

  guint dlna_opval;
  guint64 dlna_duration;				/* dlna duration */
...
//2013-03-27 gstreamer upgrade start
#ifdef INCLUDE_LMF
#define DEFAULT_DLNA_PROTOCOLINFO   NULL
#define DEFAULT_DRM_TYPE 			 -1
#define DTCPIP_MIMETYPE				"application/x-dtcp1"
...
//2013-03-27 gstreamer upgrade start
#ifdef INCLUDE_LMF
  g_object_class_install_property (gobject_class, PROP_DLNA_PROTOCOLINFO,
    g_param_spec_string ("dlna-protocolinfo", "protocolinfo",
	  "dlna protocolinfo", DEFAULT_DLNA_PROTOCOLINFO,
	  G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  
  g_object_class_install_property (gobject_class, PROP_DLNA_CLEARTEXTSIZE,
	g_param_spec_uint64 ("dlna-cleartextsize", "dlna:cleartextSize",
	  "dlna:cleartextSize", 0, G_MAXUINT64, 0,
	  G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
//2013-03-27 gstreamer upgrade end
...
//2013-03-27 gstreamer upgrade start
  ...
  /**
   * GstURIDecodeBin:n-title
   *
   * Get the total number of available title.
   */
  g_object_class_install_property (gobject_class, PROP_N_TITLE,
    g_param_spec_int ("n-title", "Number Title",
      "Total number of title", 0, G_MAXINT, 0,
      G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
  
  /**
   * GstURIDecodeBin:current-title:
   *
   * Get or set the current title
   */
  g_object_class_install_property (gobject_class, PROP_CURRENT_TITLE,
    g_param_spec_int ("current-title", "Current Title",
      "Current title",
      -1, G_MAXINT, -1, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  
  /**
   * GstURIDecodeBin:title-offset:
   */
  g_object_class_install_property (gobject_class, PROP_TITLE_OFFSET,
    g_param_spec_int64 ("title-offset", "Title Offset",
      "Title",
      G_MININT64, G_MAXINT64, 0,
      G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * GstURIDecodeBin:drm-type
   *
   * DRM-type.
   */
  g_object_class_install_property (gobject_class, PROP_DRM_TYPE,
    g_param_spec_int ("drm-type", "DRM type",
      ...
      G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * GstURIDecodeBin:thumbnail-mode
   *
   * Thumbnail mode.
   */
  ...
  g_object_class_install_property (gobject_class, PROP_DLNA_OPVAL,
    g_param_spec_uint ("dlna-opval", "DLNA OPVAL",
      "DLNA OPVal",
      0, G_MAXUINT32, 0x111, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  
  g_object_class_install_property (gobject_class, PROP_DLNA_DURATION,
    g_param_spec_uint64 ("dlna-duration", "dlan duration value",
      "dlna duration value",
      0, G_MAXUINT64, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  
  g_object_class_install_property (gobject_class, PROP_PLAYBIN_MODE,
    g_param_spec_boolean ("playbin-mode", "Playbin Mode",
      "Tracing infomation whether running in playbin2", 
      TRUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  
  g_object_class_install_property (gobject_class, PROP_VDEC_CHANNELRESERVED,
...
//2013-03-27 gstreamer upgrade start
#ifdef INCLUDE_LMF
  dec->protocol_info = g_strdup (DEFAULT_DLNA_PROTOCOLINFO);
  dec->cleartext_size = 0;
  dec->current_title = -1;
  dec->title_offset = -1;
  dec->drm_type = DEFAULT_DRM_TYPE;
  dec->thumbnail_mode = DEFAULT_THUMBNAIL_MODE;
  dec->dlna_opval = 0x111;
  dec->dlna_duration = 0; 
  dec->playbin_mode = FALSE;
  dec->vdec_channelreserved = FALSE;
#endif
//2013-03-27 gstreamer upgrade end  
...
//2013-03-27 gstreamer upgrade start
    ...
    case PROP_PLAYBIN_MODE:
      GST_OBJECT_LOCK (dec);
      dec->playbin_mode = g_value_get_boolean (value);
      GST_OBJECT_UNLOCK (dec);
      break;
    case PROP_VDEC_CHANNELRESERVED:
      GST_OBJECT_LOCK (dec);
      dec->vdec_channelreserved = g_value_get_boolean (value);
      GST_OBJECT_UNLOCK (dec);
      break;
    case PROP_DLNA_PROTOCOLINFO:
      GST_OBJECT_LOCK (dec);
      g_free (dec->protocol_info);
      dec->protocol_info = g_value_dup_string (value);
      GST_DEBUG_OBJECT (dec, "set protocolinfo : %s", dec->protocol_info);
      GST_OBJECT_UNLOCK (dec);
      break;
    case PROP_DLNA_CLEARTEXTSIZE:
      GST_OBJECT_LOCK (dec);
      dec->cleartext_size = g_value_get_uint64 (value);
      GST_DEBUG_OBJECT (dec, "set dlna:cleartextSize : %" G_GUINT64_FORMAT,
          dec->cleartext_size);
      GST_OBJECT_UNLOCK (dec);
      break;
#endif
...
//2013-03-27 gstreamer upgrade start
    ...
    case PROP_CURRENT_TITLE:
    {
      ...
      for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
        GObject *decodebin = G_OBJECT (walk->data);
        if (g_object_class_find_property (G_OBJECT_GET_CLASS (decodebin),
                "current-title")) {
          g_object_set (decodebin, "current-title", g_value_get_int (value),
              NULL);
          dec->current_title = g_value_get_int (value);
        }
      }
      GST_URI_DECODE_BIN_UNLOCK (dec);
      break;
    ...
    case PROP_TITLE_OFFSET:
    {
      ...
      for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
        ...
        if (g_object_class_find_property (G_OBJECT_GET_CLASS (decodebin),
                "title-offset")) {
          g_object_set (decodebin, "title-offset", g_value_get_int64 (value),
              NULL);
          dec->title_offset = g_value_get_int64 (value);
        }
      }
      GST_URI_DECODE_BIN_UNLOCK (dec);
      break;
    ...
    case PROP_DRM_TYPE:
      dec->drm_type = g_value_get_int (value);
      break;
    case PROP_THUMBNAIL_MODE:
      dec->thumbnail_mode = g_value_get_uint (value);
      break;
    case PROP_DLNA_OPVAL:
      dec->dlna_opval = g_value_get_uint (value);
      break;
	case PROP_DLNA_DURATION:
	  dec->dlna_duration = g_value_get_uint64 (value);
	  break;
#endif
//2013-03-27 gstreamer upgrade end	  
    default:
...
//2013-03-27 gstreamer upgrade start
	...
	case PROP_VDEC_CHANNELRESERVED:
	  GST_OBJECT_LOCK (dec);
	  g_value_set_boolean (value, dec->vdec_channelreserved);
	  GST_OBJECT_UNLOCK (dec);
	  break;
	case PROP_PLAYBIN_MODE:
	  GST_OBJECT_LOCK (dec);
	  g_value_set_boolean (value, dec->playbin_mode);
	  GST_OBJECT_UNLOCK (dec);
	  break;
	case PROP_DLNA_PROTOCOLINFO:
	  GST_OBJECT_LOCK (dec);
	  g_value_set_string (value, dec->protocol_info);
	  GST_OBJECT_UNLOCK (dec);
	  break;
	case PROP_DLNA_CLEARTEXTSIZE:
	  GST_OBJECT_LOCK (dec);
	  g_value_set_uint64 (value, dec->cleartext_size);
	  GST_OBJECT_UNLOCK (dec);
	  break;
	case PROP_DLNA_OPVAL:
	  GST_OBJECT_LOCK (dec);
	  g_value_set_uint (value, dec->dlna_opval);
	  GST_OBJECT_UNLOCK (dec);
	  break;
	case PROP_DLNA_DURATION:
	  GST_OBJECT_LOCK (dec);
	  g_value_set_uint64 (value, dec->dlna_duration);
	  GST_OBJECT_UNLOCK (dec);
	  break;
#endif
//2013-03-27 gstreamer upgrade end
...
//2013-03-27 gstreamer upgrade start
    ...
    case PROP_N_TITLE:
    {
      ...
      for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
        ...
        if (g_object_class_find_property (G_OBJECT_GET_CLASS (decodebin),
                "n-title")) {
          g_object_get (decodebin, "n-title", &nTitle, NULL);
          dec->current_title = nTitle;
        }
      }
      GST_URI_DECODE_BIN_UNLOCK (dec);
    ...
    case PROP_CURRENT_TITLE:
    {
      ...
      for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
        ...
        if (g_object_class_find_property (G_OBJECT_GET_CLASS (decodebin),
                "current-title")) {
          g_object_get (decodebin, "current-title", &(dec->current_title),
              NULL);
        }
      }
      GST_URI_DECODE_BIN_UNLOCK (dec);
      g_value_set_int (value, dec->current_title);
      break;
    }
    case PROP_TITLE_OFFSET:
    {
      ...
      for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
        GObject *decodebin = G_OBJECT (walk->data);
        g_object_get (decodebin, "title-offset", &(dec->title_offset), NULL);
      }
      GST_URI_DECODE_BIN_UNLOCK (dec);
      g_value_set_int64 (value, dec->title_offset);

      break;
    }
    case PROP_DRM_TYPE:
      g_value_set_int (value, dec->drm_type);
      break;
    case PROP_THUMBNAIL_MODE:
      GST_OBJECT_LOCK (dec);
      g_value_set_uint (value, dec->thumbnail_mode);
      GST_OBJECT_UNLOCK (dec);
      break;
#endif
//2013-03-27 gstreamer upgrade end	  
...
//2013-03-27 gstreamer upgrade start
static const gchar *stream_uris[] = { "mms://",
...
static const gchar *queue_uris[] = { "http://", "https://", "cdda://", NULL };
//2013-03-27 gstreamer upgrade end
...
//2013-03-27 gstreamer upgrade start		
        if (*is_raw || use_queue) {
          GstElement *outelem;
          GST_DEBUG_OBJECT (decoder, "using queue2");
//2013-03-27 gstreamer upgrade end
...
#ifdef INCLUDE_LMF
  if (decoder->uri)
	g_object_set (decodebin, "drm-uri", decoder->uri, NULL);
    
  if (decoder->current_title != -1)
	g_object_set (decoder, "current-title", decoder->current_title, NULL);
  
  if (decoder->title_offset != -1)
	g_object_set (decoder, "title-offset", decoder->title_offset, NULL);
  
  g_object_set (decodebin, "thumbnail-mode", decoder->thumbnail_mode, NULL);
  g_object_set (decodebin, "dlna-protocolinfo", decoder->protocol_info, NULL);
  g_object_set (decodebin, "dlna-opval", decoder->dlna_opval, NULL);
  g_object_set (decodebin, "playbin-mode", decoder->playbin_mode, NULL);
  g_object_set (decodebin, "vdec-channelreserved", decoder->vdec_channelreserved, NULL);
  g_object_set (decodebin, "dlna-duration", decoder->dlna_duration, NULL);
  
	if (decoder->drm_type != DEFAULT_DRM_TYPE)
      g_object_set (decodebin, "drm-type", decoder->drm_type, NULL);
#endif
//2013-03-27 gstreamer upgrade end
...
//2013-03-27 gstreamer upgrade start
  GST_DEBUG_OBJECT (queue, "setting up max buffer size for queue2");
//2013-03-27 gstreamer upgrade end  
...
#ifdef INCLUDE_LMF
  GstElement *dtcpip;
  GstElement *temp_element;
  
  dtcpip = NULL;
  
  /* now create the decoder element */
  if (decoder->protocol_info) {
    GST_INFO_OBJECT (decoder, "check DTCP-IP is used ...");
    GST_DEBUG_OBJECT (decoder, "checking mimetype : %s",
      decoder->protocol_info);
    if (strstr (decoder->protocol_info, DTCPIP_MIMETYPE)) {
      GST_DEBUG_OBJECT (decoder, "create dtcpip plugin");
      dtcpip = gst_element_factory_make ("dtcpip", NULL);
      if (!dtcpip)
        goto no_dtcpip;
  
      if (g_object_class_find_property (G_OBJECT_GET_CLASS (decoder->source),
        "is-dtcp")) {
        GST_INFO_OBJECT (decoder, "set DTCP-IP is used ...");
        g_object_set (decoder->source, "is-dtcp", TRUE, NULL);
    }
  
    g_object_set (dtcpip, "protocolinfo", decoder->protocol_info, NULL);
    g_object_set (dtcpip, "cleartextsize", decoder->cleartext_size, NULL);
  
    gst_bin_add (GST_BIN_CAST (decoder), dtcpip);
  
    if (!gst_element_link_pads (decoder->source, NULL, dtcpip, "sink"))
      goto could_not_link;
    } else
    GST_DEBUG_OBJECT (decoder, "no need of dtcpip plugin");
  } else
  GST_DEBUG_OBJECT (decoder, "no mimetype");
#endif
//2013-03-27 gstreamer upgrade end
...
//2013-03-27 gstreamer upgrade start
	...
	if (dtcpip)
	  ...
	  temp_element = decoder->source;
  
	if (!gst_element_link_pads (temp_element, NULL, typefind, "sink"))
	  goto could_not_link;
#else
...
no_dtcpip:
  {
    post_missing_plugin_error (GST_ELEMENT_CAST (decoder), "dtcpip");
    return FALSE;
  }
#endif
//2013-03-27 gstreamer upgrade end
...
//2013-03-27 gstreamer upgrade start
  GST_DEBUG_OBJECT (bin, "Found new pad %s.%s in source element %s",
    GST_DEBUG_PAD_NAME (pad), gst_element_get_name (element));
//2013-03-27 gstreamer upgrade end
