...
//2013-03-27 gstreamer upgrade start
#ifndef INCLUDE_LMF
#define INCLUDE_LMF
#endif
//2013-03-27 gstreamer upgrade end
...
//2013-03-27 gstreamer upgrade start
  ...
  char *dlna_protocolinfo;
  guint dlna_opval;
  guint64 dlna_duration;
  gboolean playbin_mode;
...
//2013-03-27 gstreamer upgrade start
#if defined GST_PLATFORM && GST_PLATFORM == 1
#warning "decodebin will use extended buffer instead of origial size."

#define AUTO_PLAY_SIZE_BYTES        6 * 1024 * 1024
#define AUTO_PLAY_SIZE_BUFFERS      5
#define AUTO_PLAY_SIZE_TIME         0 * GST_SECOND
...
#define DEFAULT_THUMBNAIL_MODE		0
#define DEFAULT_PROTOCO_INFO	NULL
#define DEFAULT_COUNTRY_INFO	NULL
#endif
//2013-03-27 gstreamer upgrade end
...
//2013-03-27 gstreamer upgrade start
#ifdef INCLUDE_LMF
  /**
   * GstDecodeBin2::drm-uri
   *
   * uri used in drm plugin.
   *
   * Since: 0.10.30
   */
  g_object_class_install_property (gobject_klass, PROP_DRM_URI,
      g_param_spec_string ("drm-uri", "drm-uri",
          "uri used in drm plugin",
          NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));


  /**
   * GstDecodeBin2:n-title
   *
  ...
  g_object_class_install_property (gobject_klass, PROP_PROTOCOL_INFO,
      g_param_spec_string ("dlna-protocolinfo", "dlna-protocolinfo",
          "dlna-protocolinfo",
          NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_klass, PROP_DLNA_OPVAL,
      g_param_spec_uint ("dlna-opval", "DLNA OPVAL",
          "DLNA OPVal",
  ...
  g_object_class_install_property (gobject_klass, PROP_DLNA_DURATION,
      g_param_spec_uint64 ("dlna-duration", "dlan duration value",
          "dlna duration",
          0, G_MAXUINT64, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_klass, PROP_PLAYBIN_MODE,
      g_param_spec_boolean ("playbin-mode", "Playbin Mode",
          "Playbin Mode", FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_klass, PROP_VDEC_CHANNELRESERVED,
      g_param_spec_boolean ("vdec-channelreserved", "Vdec Channel Reserved",
          "Vdec Channel Reserved",
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_klass, PROP_COUNTRY_INFO,
      g_param_spec_string ("country-info", "country-info",
          "country-info",
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
//2013-03-27 gstreamer upgrade start
#ifdef INCLUDE_LMF
  decode_bin->thumbnail_mode = DEFAULT_THUMBNAIL_MODE;
  decode_bin->dlna_protocolinfo = DEFAULT_PROTOCO_INFO;
  decode_bin->dlna_opval = 0x111;
  decode_bin->dlna_duration = 0;
  decode_bin->playbin_mode = FALSE;
  decode_bin->vdec_channelreserved = FALSE;
  decode_bin->country_info = DEFAULT_COUNTRY_INFO;
#endif
//2013-03-27 gstreamer upgrade end
}
...
//2013-03-27 gstreamer upgrade start
#ifdef INCLUDE_LMF
  g_free (decode_bin->drm_uri);
  g_free (decode_bin->dlna_protocolinfo);
  g_free (decode_bin->country_info);
#endif
//2013-03-27 gstreamer upgrade end
...
//2013-03-27 gstreamer upgrade start
#ifdef INCLUDE_LMF
    case PROP_PLAYBIN_MODE:
      dbin->playbin_mode = g_value_get_boolean (value);
      break;
    case PROP_VDEC_CHANNELRESERVED:
      dbin->vdec_channelreserved = g_value_get_boolean (value);
      break;
    case PROP_DLNA_OPVAL:
    {
      GList *walk;
      GST_OBJECT_LOCK (dbin);
      dbin->dlna_opval = g_value_get_uint (value);
      if (dbin->decode_chain) {
        for (walk = dbin->decode_chain->elements; walk;
            walk = g_list_next (walk)) {
          GstElement *element = GST_ELEMENT (walk->data);
          if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
                  "dnla-opval"))
            g_object_set (element, "dlna-opval", dbin->dlna_opval, NULL);
        }
      }
      GST_OBJECT_UNLOCK (dbin);
    }
	...
	case PROP_DLNA_DURATION:
	  dbin->dlna_duration = g_value_get_uint64 (value);
	  break;
    case PROP_DRM_URI:
      GST_OBJECT_LOCK (dbin);
      g_free (dbin->drm_uri);
      dbin->drm_uri = g_value_dup_string (value);
      GST_OBJECT_UNLOCK (dbin);
      break;
    case PROP_CURRENT_TITLE:
    {
      ...
      if (dbin->decode_chain) {
        for (walk = dbin->decode_chain->elements; walk;
          ...
          if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
                  "current-title")) {
            g_object_set (element, "current-title", g_value_get_int (value),
                NULL);
            dbin->current_title = g_value_get_int (value);
          }
        }
      }
    ...
    case PROP_TITLE_OFFSET:
    {
      ...
      /* get the property on all decodebins now */
      if (dbin->decode_chain) {
        for (walk = dbin->decode_chain->elements; walk;
            walk = g_list_next (walk)) {
          GstElement *element = GST_ELEMENT (walk->data);
          if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
                  "title-offset")) {
            g_object_set (element, "title-offset", g_value_get_int64 (value),
                NULL);
            dbin->title_offset = g_value_get_int64 (value);
          }
        }
      }
    ...
    case PROP_DRM_TYPE:
      dbin->drm_type = g_value_get_int (value);
      break;
    case PROP_THUMBNAIL_MODE:
      dbin->thumbnail_mode = g_value_get_uint (value);
      break;

    case PROP_PROTOCOL_INFO:
      if (dbin->dlna_protocolinfo)
        g_free (dbin->dlna_protocolinfo);
      dbin->dlna_protocolinfo = g_value_dup_string (value);
      break;
    case PROP_COUNTRY_INFO:
      if (dbin->country_info)
        g_free (dbin->country_info);
      dbin->country_info = g_value_dup_string (value);
      break;
#endif
//2013-03-27 gstreamer upgrade end
    default:
...
//2013-03-27 gstreamer upgrade start
    ...
    case PROP_VDEC_CHANNELRESERVED:
      GST_OBJECT_LOCK (dbin);
      g_value_set_boolean (value, dbin->vdec_channelreserved);
      GST_OBJECT_UNLOCK (dbin);
      break;
    case PROP_PLAYBIN_MODE:
      GST_OBJECT_LOCK (dbin);
      g_value_set_boolean (value, dbin->playbin_mode);
      GST_OBJECT_UNLOCK (dbin);
      break;
    case PROP_DLNA_OPVAL:
      GST_OBJECT_LOCK (dbin);
      g_value_set_uint (value, dbin->dlna_opval);

      GST_OBJECT_UNLOCK (dbin);
      break;
    case PROP_DLNA_DURATION:
      GST_OBJECT_LOCK (dbin);
      g_value_set_uint64 (value, dbin->dlna_duration);

      GST_OBJECT_UNLOCK (dbin);
	  break;
    case PROP_DRM_URI:
      GST_OBJECT_LOCK (dbin);
      g_value_set_string (value, dbin->drm_uri);
      GST_OBJECT_UNLOCK (dbin);
      break;
    case PROP_N_TITLE:
    {
      ...
      if (dbin->decode_chain) {
        for (walk = dbin->decode_chain->elements; walk;
          ...
          if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
                  "n-title"))
            g_object_get (element, "n-title", &nTitle, NULL);

        }
      }
      GST_OBJECT_UNLOCK (dbin);

      g_value_set_uint (value, nTitle);
      break;
    }
    case PROP_CURRENT_TITLE:
    {
      ...
      if (dbin->decode_chain) {
        for (walk = dbin->decode_chain->elements; walk;
          ...
          if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
                  "current-title"))
            g_object_get (element, "current-title", &current_title, NULL);
        }
      }
      GST_OBJECT_UNLOCK (dbin);
      g_value_set_int (value, current_title);
      break;
    }
    case PROP_TITLE_OFFSET:
    {
      ...
      if (dbin->decode_chain) {
        for (walk = dbin->decode_chain->elements; walk;
          ...
          if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
                  "title-offset"))
            g_object_get (element, "title-offset", &title_offset, NULL);
        }
      }
      GST_OBJECT_UNLOCK (dbin);
      g_value_set_int64 (value, title_offset);

      break;
    }
    case PROP_DRM_TYPE:
      g_value_set_int (value, dbin->drm_type);

      break;
    case PROP_THUMBNAIL_MODE:
      GST_OBJECT_LOCK (dbin);
      g_value_set_uint (value, dbin->thumbnail_mode);
      GST_OBJECT_UNLOCK (dbin);
      break;
    case PROP_PROTOCOL_INFO:
      GST_OBJECT_LOCK (dbin);
      g_value_set_string (value, dbin->dlna_protocolinfo);
      GST_OBJECT_UNLOCK (dbin);
      break;
    case PROP_COUNTRY_INFO:
      GST_OBJECT_LOCK (dbin);
      g_value_set_string (value, dbin->country_info);
      GST_OBJECT_UNLOCK (dbin);
      break;	
...
//2013-03-27 gstreamer upgrade start
    ...
    if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
            "current-title"))
      g_object_set (element, "current-title", dbin->current_title, NULL);

    if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
            "title-offset"))
      g_object_set (element, "title-offset", dbin->title_offset, NULL);

    if (dbin->drm_type != DEFAULT_DRM_TYPE
        && g_object_class_find_property (G_OBJECT_GET_CLASS (element),
            "drm-type")) {
      g_object_set (element, "drm-type", dbin->drm_type, NULL);
      GST_DEBUG_OBJECT (dbin, "set %s'drm type to  %d",
          gst_plugin_feature_get_name (GST_PLUGIN_FEATURE (factory)),
          dbin->drm_type);
    }
    if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
            "playbin-mode"))
      g_object_set (element, "playbin-mode", dbin->playbin_mode, NULL);

    if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
            "vdec-channelreserved"))
      g_object_set (element, "vdec-channelreserved", dbin->vdec_channelreserved,
          NULL);

    if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
            "thumbnail-mode"))
      g_object_set (element, "thumbnail-mode", dbin->thumbnail_mode, NULL);
    ...
    if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
            "dlna-protocolinfo"))
      g_object_set (element, "dlna-protocolinfo", dbin->dlna_protocolinfo,
          NULL);
    if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
            "dlna-duration"))
      g_object_set (element, "dlna-duration", dbin->dlna_duration,
          NULL);

    if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
            "country-info"))
      g_object_set (element, "country-info", dbin->country_info,
          NULL);
...
//2013-03-27 gstreamer upgrade start
      gst_element_get_name (element), chain);
//2013-03-27 gstreamer upgrade end
...
//2013-03-27 gstreamer upgrade start
static GstElement *
autoplug_DRM (GstPad * srcpad, GstDecodeBin * decode_bin)
{
  ...
  GST_DEBUG_OBJECT (decode_bin, "autoplug_DRM ");

  caps = gst_pad_get_caps_reffed (srcpad);


  capsstr = gst_caps_to_string (caps);
  GST_DEBUG_OBJECT (decode_bin, "typefinder src pad :%p, caps: %s", srcpad,
      capsstr);
  g_free (capsstr);


  g_mutex_lock (decode_bin->factories_lock);
  elements =
      gst_element_factory_list_get_elements (GST_ELEMENT_FACTORY_TYPE_DRM,
      GST_RANK_NONE);
  elements = g_list_sort (elements, gst_plugin_feature_rank_compare_func);
  GST_DEBUG_OBJECT (decode_bin, "found DRM factories %" GST_PTR_FORMAT ": %d",
      elements, g_list_length (elements));
  mylist =
      gst_element_factory_list_filter (elements, caps, GST_PAD_SINK, FALSE);
  g_mutex_unlock (decode_bin->factories_lock);



  GST_DEBUG_OBJECT (decode_bin, "found factories %" GST_PTR_FORMAT ": %d",
      mylist, g_list_length (mylist));

  GST_PLUGIN_FEATURE_LIST_DEBUG (mylist);

  if (mylist == NULL) {
    GST_DEBUG_OBJECT (decode_bin, "No DRM element for caps: %" GST_PTR_FORMAT,
        caps);
    gst_caps_unref (caps);
  ...
  for (tmp = mylist; tmp; tmp = g_list_next (tmp)) {
    ...
    GST_DEBUG_OBJECT (decode_bin, "Element list : name : %s",
        gst_plugin_feature_get_name (GST_PLUGIN_FEATURE (factory)));
    ...
    /* Filter errors, this will prevent the element from causing the pipeline
     * to error while we test it using READY state. */
    add_error_filter (decode_bin, element);

    /* ... add it ... */
    if (!(gst_bin_add (GST_BIN_CAST (decode_bin), element))) {
      GST_WARNING_OBJECT (decode_bin, "Couldn't add %s to the bin",
          gst_element_get_name (element));
      remove_error_filter (decode_bin, element);
      gst_object_unref (element);
      continue;
    }

    if (decode_bin->drm_uri)
      g_object_set (element, "msdrm_set_url", decode_bin->drm_uri, NULL);

    /* Find its sink pad. */
    if (!(sinkpad = find_sink_pad (element))) {
      GST_WARNING_OBJECT (decode_bin, "Element %s doesn't have a sink pad",
          gst_element_get_name (element));
      remove_error_filter (decode_bin, element);
    ...
    if ((gst_pad_link (srcpad, sinkpad)) != GST_PAD_LINK_OK) {
      GST_WARNING_OBJECT (decode_bin, "Link failed on pad %s:%s",
          GST_DEBUG_PAD_NAME (sinkpad));
      remove_error_filter (decode_bin, element);
      gst_object_unref (sinkpad);
      gst_bin_remove (GST_BIN (decode_bin), element);
    ...
    GST_LOG_OBJECT (decode_bin, "linked on pad %s:%s",
...
//2013-03-27 gstreamer upgrade start
#ifdef INCLUDE_LMF
  DRM_element = autoplug_DRM (pad, decode_bin);

  if (DRM_element) {
    drm_pad = gst_element_get_static_pad (DRM_element, "src");
    decode_bin->decode_chain = gst_decode_chain_new (decode_bin, NULL, drm_pad);

    analyze_new_pad (decode_bin, DRM_element, drm_pad, caps,
...
//2013-03-27 gstreamer upgrade start
      {
        ...
        if (dbin->decode_chain) {
          for (walk = dbin->decode_chain->elements; walk;
              walk = g_list_next (walk)) {
            GstElement *el = GST_ELEMENT (walk->data);
            if (g_object_class_find_property (G_OBJECT_GET_CLASS (el),
                    "dnla-opval"))
              g_object_set (el, "dlna-opval", dbin->dlna_opval, NULL);
          }
        }
        DYN_UNLOCK (dbin);
      }
