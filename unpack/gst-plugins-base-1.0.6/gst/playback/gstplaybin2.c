...
//2013-03-26 gstreamer upgrade start
  GHashTable *padpairs;
  guint npadpairs;
//2013-03-26 gstreamer upgrade end
};
...
//2013-03-26 gstreamer upgrade start
	...
	guint64 cleartext_size;
	gboolean vdec_channelreserved;
#endif
	guint32 opval;
	guint32 flagval;
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
  guint thumbnail_mode;         /* set to TRUE when playbin2 is in thumbnail mode */
  gint drm_type;                /* drm type */
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF              // NetCast 3.0
#define DEFAULT_DRM_TYPE       -1
#endif
//2013-03-26 gstreamer upgrade end
...
//2013-03-26 gstreamer upgrade start  
  ...
  PROP_TITLE_OFFSET,
  PROP_DRM_TYPE,
  PROP_DLNA_PROTOCOLINFO,
  PROP_DLNA_CONTENTLENGTH,
  PROP_DLNA_CLEARTEXTSIZE,
  PROP_DLNA_OPVAL,
  PROP_DLNA_FLAGVAL,
  PROP_DLNA_DURATION,
  PROP_VDEC_CHANNELRESERVED,
#endif  
//2013-03-26 gstreamer upgrade end
...
static gboolean gst_play_bin_send_event (GstElement * element,
    GstEvent * event);
static gboolean gst_play_bin_send_event_to_sink (GstPlayBin * playbin,
    GstEvent * event);

static void do_async_start (GstPlayBin * playbin);
static void do_async_done (GstPlayBin * playbin);
#endif
//2013-03-26 gstreamer upgrade end
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF              // NetCast 3.0
  /**
	 * GstPlayBin2:thumbnail-mode
	 *
	 * Get or set the thumbnail mode
	 */
  g_object_class_install_property (gobject_klass, PROP_THUMBNAIL_MODE,
      g_param_spec_uint ("thumbnail-mode", "Thumbnail mode",
          "Thumbnail mode", 0, 2, 0,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * GstPlayBin2:drm-type
   *
   * DRM-type.
   */
  g_object_class_install_property (gobject_klass, PROP_DRM_TYPE,
      g_param_spec_int ("drm-type", "DRM type",
          "DRM type >= 0", -1, G_MAXINT, DEFAULT_DRM_TYPE,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   * GstPlayBin2:n-title
   *
   * Get the total number of available title.
   */
  g_object_class_install_property (gobject_klass, PROP_N_TITLE,
      g_param_spec_int ("n-title", "Number Title",
          "Total number of title", 0, G_MAXINT, 0,
          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

  /**
   * GstPlayBin2:current-title:
   *
   * Get or set the current title
   */
  g_object_class_install_property (gobject_klass, PROP_CURRENT_TITLE,
      g_param_spec_int ("current-title", "Current Title",
          "Current title",
          -1, G_MAXINT, -1, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  /**
   * GstPlayBin2:title-offset:
   */
  g_object_class_install_property (gobject_klass, PROP_TITLE_OFFSET,
      g_param_spec_int64 ("title-offset", "Title Offset",
          "Title",
          G_MININT64, G_MAXINT64, 0,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  /**
   * GstPlayBin2:dlna-protocolInfo:
   */
  g_object_class_install_property (gobject_klass, PROP_DLNA_PROTOCOLINFO,
      g_param_spec_string ("dlna-protocolinfo",
          "DLNA PROTOCOLINFO",
          "PROTOCOLFINFO"
          "to be used for DLNA", NULL,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * GstPlayBin2:dlna-contentlength
   */
  g_object_class_install_property (gobject_klass, PROP_DLNA_CONTENTLENGTH,
      g_param_spec_uint64 ("dlna-contentlength",
          "Content length(bytes)",
          "Content length(bytes)",
          0, G_MAXUINT64, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * GstPlayBin2:dlna-cleartextSize
   */
  g_object_class_install_property (gobject_klass, PROP_DLNA_CLEARTEXTSIZE,
      g_param_spec_uint64 ("dlna-cleartextsize",
          "dlna:cleartextSize",
          "dlna:cleartextSize for DTCP-IP content",
          0, G_MAXUINT64, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * GstPlayBin2:dlna-opval
   */
  g_object_class_install_property (gobject_klass, PROP_DLNA_OPVAL,
      g_param_spec_uint ("dlna-opval",
          "dlna op value",
          "dlna op value",
          0, G_MAXUINT32, 0x111, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * GstPlayBin2:dlna-flagval
   */
  g_object_class_install_property (gobject_klass, PROP_DLNA_FLAGVAL,
      g_param_spec_uint ("dlna-flagval",
          "dlna flag value",
          "dlna flag value",
          0, G_MAXUINT32, 0x111, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_klass, PROP_DLNA_DURATION,
      g_param_spec_uint64 ("dlna-duration",
          "dlna duration value",
          "dlna duration value",
          0, G_MAXUINT64, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));


  g_object_class_install_property (gobject_klass, PROP_VDEC_CHANNELRESERVED,
      g_param_spec_boolean ("vdec-channelreserved",
          "Vdec channel reserved",
          "Vdec channel reserved",
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
  group->selector[PLAYBIN_STREAM_AUDIO].padpairs = NULL;
  group->selector[PLAYBIN_STREAM_VIDEO].padpairs = NULL;
  group->selector[PLAYBIN_STREAM_TEXT].padpairs = NULL;

  group->audio_sink = NULL;
  group->video_sink = NULL;

  /* mkv certi. */
  group->current_title = -1;
  group->title_offset = -1;

  /*dlna property */
  group->protocol_info = NULL;
  group->content_length = 0;
  group->cleartext_size = 0;
  group->opval = 0x111;
  group->flagval = 0x111;
  group->dlna_duration = 0;
  group->vdec_channelreserved = FALSE;
#endif
//2013-03-26 gstreamer upgrade end
}
...
gst_play_bin_update_elements_list (GstPlayBin * playbin)
{
  ...
  if (!playbin->elements || playbin->elements_cookie != cookie) {
...
#endif
    GST_DEBUG_OBJECT (playbin, "number of (parse + demuxer+ demuxer) = %d",
        g_list_length (res));
    ...
    GST_DEBUG_OBJECT (playbin, "number of sinks = %d", g_list_length (tmp));

    playbin->elements = g_list_concat (res, tmp);
    playbin->elements = g_list_sort (playbin->elements, compare_factories_func);
    GST_DEBUG_OBJECT (playbin, "all autopluggable elements  = %d",
      g_list_length (playbin->elements));
...
gst_play_bin_init (GstPlayBin * playbin)
{
  ...
  //2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
  GST_DEBUG ("LMF is skipping a logic to set playsink");
#else
  /* add sink */
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
  playbin->thumbnail_mode = 0;

  playbin->video_sink = NULL;
  playbin->audio_sink = NULL;
  playbin->text_sink = NULL;
  /* Default value of drm_type means no use of drm */
  playbin->drm_type = DEFAULT_DRM_TYPE;
#endif
//2013-03-26 gstreamer upgrade end  
}
...
gst_play_bin_finalize (GObject * object)
{
  ...
  //2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
  if (playbin->video_sink && GST_IS_ELEMENT( playbin->video_sink ) &&
	G_OBJECT(playbin->video_sink )->ref_count > 0 ) {
	GST_INFO_OBJECT(playbin, "\n++++[final] [%p] playbin->video_sink->ref_count = [%d]++++\n",
	  playbin->video_sink, G_OBJECT(playbin->video_sink)->ref_count);
	gst_element_set_state (playbin->video_sink, GST_STATE_NULL);
	gst_object_unref (playbin->video_sink);
  }
  if (playbin->audio_sink && GST_IS_ELEMENT( playbin->audio_sink ) &&
    G_OBJECT(playbin->audio_sink )->ref_count > 0 ) {
	GST_INFO_OBJECT(playbin, "\n++++[final] [%p] playbin->audio_sink->ref_count = [%d]++++\n",
	  playbin->audio_sink, G_OBJECT(playbin->audio_sink)->ref_count);
	gst_element_set_state (playbin->audio_sink, GST_STATE_NULL);
	gst_object_unref (playbin->audio_sink);
  }
  if (playbin->text_sink && GST_IS_ELEMENT( playbin->text_sink ) &&
    G_OBJECT(playbin->text_sink )->ref_count > 0 ) {
	GST_INFO_OBJECT(playbin, "\n++++[final] [%p] playbin->text_sink->ref_count = [%d]++++\n",
	  playbin->text_sink, G_OBJECT(playbin->text_sink)->ref_count);
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
  GST_DEBUG ("LMF is skipping configuration");
#else
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
  {
    ...
    selpad = gst_ghost_pad_get_target (GST_GHOST_PAD_CAST (sinkpad));
    g_object_get (selpad, "tags", &result, NULL);
    g_object_unref (selpad);
  }
#else
...
gst_play_bin_convert_sample (GstPlayBin * playbin, GstCaps * caps)
{
	  ...
	  if (!playbin->video_sink) {
		GST_DEBUG_OBJECT (playbin, "no video sink!");
		return NULL;
	  }
	
	  GST_DEBUG_OBJECT (playbin, "convert-frame");
	  g_signal_emit_by_name (G_OBJECT (playbin->video_sink), "convert-frame", caps,
		  &result);
...
//2013-03-26 gstreamer upgrade start		
#ifdef INCLUDE_LMF
	  GST_DEBUG ("LMF doesn't set vis plugin");
#else
...
//2013-03-26 gstreamer upgrade start				
#ifdef INCLUDE_LMF
	  GST_DEBUG ("LMF doesn't set volume");
#else
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
      GST_DEBUG ("LMF doesn't set mute");
#else
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
	  GST_DEBUG ("LMF doesn't set font description");
#else
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
	  GST_DEBUG ("LMF doesn't set av offset");
#else
...
//2013-03-26 gstreamer upgrade start
    ...
    case PROP_THUMBNAIL_MODE:
      playbin->thumbnail_mode = g_value_get_uint (value);
      break;
    case PROP_DRM_TYPE:
      playbin->drm_type = g_value_get_int (value);
      break;
    case PROP_CURRENT_TITLE:
    {
      ...
      if (group->uridecodebin)
        g_object_set (group->uridecodebin, "current-title",
            g_value_get_int (value), NULL);

      group->current_title = g_value_get_int (value);

      GST_PLAY_BIN_UNLOCK (playbin);
      break;
    }
    case PROP_TITLE_OFFSET:
    {
      GstSourceGroup *group;
      ...
      if (group->uridecodebin)
        g_object_set (group->uridecodebin, "title-offset",
            g_value_get_int64 (value), NULL);

      group->title_offset = g_value_get_int64 (value);

      GST_PLAY_BIN_UNLOCK (playbin);
      break;
    }
    case PROP_DLNA_PROTOCOLINFO:
    {
      GstSourceGroup *group;
      GST_PLAY_BIN_LOCK (playbin);
      group = playbin->next_group;
      ...
      g_free (group->protocol_info);
      group->protocol_info = g_value_dup_string (value);
      GST_DEBUG_OBJECT (playbin, "group: %p", group);
      GST_DEBUG_OBJECT (playbin, "set dlna protocol : %s",
          group->protocol_info);
      GST_PLAY_BIN_UNLOCK (playbin);
      break;
    }
    case PROP_DLNA_CONTENTLENGTH:
    {
      ...
      group->content_length = g_value_get_uint64 (value);
      GST_DEBUG_OBJECT (playbin, "group: %p", group);
      GST_DEBUG_OBJECT (playbin, "set content length : %" G_GUINT64_FORMAT,
          group->content_length);
      GST_PLAY_BIN_UNLOCK (playbin);
      break;
    }
    case PROP_DLNA_CLEARTEXTSIZE:
    {
      GstSourceGroup *group;
      GST_PLAY_BIN_LOCK (playbin);
      group = playbin->next_group;

      group->cleartext_size = g_value_get_uint64 (value);
      GST_DEBUG_OBJECT (playbin, "group: %p", group);
      GST_DEBUG_OBJECT (playbin, "set dlna:cleartextSize : %" G_GUINT64_FORMAT,
          group->cleartext_size);
      GST_PLAY_BIN_UNLOCK (playbin);
      break;
    }
    case PROP_DLNA_OPVAL:
    {
      ...
      group->opval = g_value_get_uint (value);
      GST_DEBUG_OBJECT (playbin, "group: %p", group);
      GST_DEBUG_OBJECT (playbin, "set op value : %" G_GUINT32_FORMAT,
          group->opval);

      if (group->uridecodebin)
        g_object_set (group->uridecodebin, "dlna-opval",
    ...
    case PROP_DLNA_FLAGVAL:
    {
      ...
      group->flagval = g_value_get_uint (value);
      GST_DEBUG_OBJECT (playbin, "group: %p", group);
      GST_DEBUG_OBJECT (playbin, "set flag value : %" G_GUINT32_FORMAT,
          group->flagval);
      GST_PLAY_BIN_UNLOCK (playbin);
      break;
    }
    case PROP_DLNA_DURATION:
    {
      ...
      group->dlna_duration = g_value_get_uint64 (value);
      GST_DEBUG_OBJECT (playbin, "group: %p", group);
      GST_DEBUG_OBJECT (playbin, "set duration value : %" G_GUINT64_FORMAT,
          group->dlna_duration);
      GST_PLAY_BIN_UNLOCK (playbin);
      break;

    }
    case PROP_VDEC_CHANNELRESERVED:
    {
      GstSourceGroup *group;
      GST_PLAY_BIN_LOCK (playbin);
      group = playbin->next_group;
      group->vdec_channelreserved = g_value_get_boolean (value);
      GST_PLAY_BIN_UNLOCK (playbin);
      break;
    }	  
//2013-03-26 gstreamer upgrade end
...
//2013-03-26 gstreamer upgrade start
  GST_DEBUG_OBJECT (playbin, "%s sink %"
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
	  GST_DEBUG ("LMF ignores to set subtitle encoding");
#else
...
//2013-03-26 gstreamer upgrade start
	case PROP_THUMBNAIL_MODE:
	  g_value_set_uint (value, playbin->thumbnail_mode);
	  break;
	case PROP_DRM_TYPE:
	  g_value_set_int (value, playbin->drm_type);
	  break;
	...
	case PROP_N_TITLE:
	{
	  ...
	  if (group->uridecodebin
		  ...
		  g_object_class_find_property (G_OBJECT_GET_CLASS
			  (group->uridecodebin), "n-title"))
		g_object_get (group->uridecodebin, "n-title", &n_title, NULL);
	  else
		n_title = -1;

	  g_value_set_int (value, n_title);
	  GST_PLAY_BIN_UNLOCK (playbin);
	  break;
	}
	case PROP_CURRENT_TITLE:
	{
	  ...
	  if (group->uridecodebin
		  &&
		  g_object_class_find_property (G_OBJECT_GET_CLASS
			  (group->uridecodebin), "current-title"))
		g_object_get (group->uridecodebin, "current-title",
			&(group->current_title), NULL);

	  g_value_set_int (value, group->current_title);
	  GST_PLAY_BIN_UNLOCK (playbin);
	  break;
	}
	case PROP_TITLE_OFFSET:
	{
	  ...
	  if (group->uridecodebin
		  &&
		  g_object_class_find_property (G_OBJECT_GET_CLASS
			  (group->uridecodebin), "title-offset"))
		g_object_get (group->uridecodebin, "title-offset",
			&(group->title_offset), NULL);

	  g_value_set_int64 (value, group->title_offset);
	  GST_PLAY_BIN_UNLOCK (playbin);
	  break;
	}
	case PROP_VDEC_CHANNELRESERVED:
	{
	  ...
	  GST_PLAY_BIN_LOCK (playbin);
	  group = get_group (playbin);
	  g_value_set_boolean (value, group->vdec_channelreserved);
	  GST_PLAY_BIN_UNLOCK (playbin);
	  break;
	}
//2013-03-26 gstreamer upgrade end
...
//2013-03-26 gstreamer upgrade start
  GST_DEBUG_OBJECT (playbin, "Active-pad: %d", select->type);
//2013-03-26 gstreamer upgrade end
...
static GstPad *
autoplug_decoder (GstPad * pad, GstCaps * caps, GstSourceGroup * group)
{
  GstPlayBin *playbin;
  GstElement *element;
  GList *mylist = NULL, *tmp, *elements;;
  GstPad *sinkpad;
  ...
  mylist =
      gst_element_factory_list_filter (elements, caps, GST_PAD_SINK, FALSE);
  g_mutex_unlock (playbin->elements_lock);

  GST_DEBUG_OBJECT (playbin, "found factories %" GST_PTR_FORMAT " : %d", mylist,
      g_list_length (mylist));

  GST_PLUGIN_FEATURE_LIST_DEBUG (mylist);

  //no decoder or
  if (mylist == NULL) {
    GST_DEBUG_OBJECT (playbin, "No need of decoder");
    gst_plugin_feature_list_free (elements);
    return pad;
  }

  for (tmp = mylist; tmp; tmp = g_list_next (tmp)) {
    ...
    gst_object_unref (sinkpad);
    GST_DEBUG_OBJECT (playbin, "linked on pad %s:%s", GST_DEBUG_PAD_NAME (pad));
    ...
    gst_plugin_feature_list_free (elements);
    return gst_element_get_static_pad (element, "src");
  }
...
static GstPad *
autoplug_sink (GstSourceSelect * select, GstPlayBin * playbin,
    GstPad * lgsbin_srcpad)
{
  GList *mylist = NULL, *tmp, *elements;
  ...
  gboolean bypass;
#endif
  GST_DEBUG_OBJECT (playbin, "select->Channel length : %d ",
      select->channels->len);

  defaultpad = g_ptr_array_index (select->channels, 0);
  caps = gst_pad_peer_get_caps (defaultpad);

  capsstr = gst_caps_to_string (caps);
  GST_DEBUG_OBJECT (playbin, "inputselector sinkp pad :%p, peer pad's caps: %s",
      defaultpad, capsstr);
  g_free (capsstr);

  /*
   * if sinks exist, use the sinks
   * Otherwise, autoplug a sink.
   */
  switch (select->type) {
    case GST_PLAY_SINK_TYPE_AUDIO:
    case GST_PLAY_SINK_TYPE_AUDIO_RAW:
      element = playbin->audio_sink;
      break;
    case GST_PLAY_SINK_TYPE_VIDEO:
    case GST_PLAY_SINK_TYPE_VIDEO_RAW:
      element = playbin->video_sink;
      break;
    case GST_PLAY_SINK_TYPE_TEXT:
      element = playbin->text_sink;
      break;
    default:
      break;
  }
  g_object_get (select->selector, "bypass", &bypass, NULL);
#ifdef INCLUDE_LMF
  if (element && !bypass) {
#else
  if (element) {
#endif
    GST_DEBUG_OBJECT (playbin,
        "sink element set by LMF %" GST_PTR_FORMAT ": %d", element);

    /* Thumbnail setting of video sink */
    if (g_object_class_find_property (G_OBJECT_GET_CLASS (element),
            "thumbnail-mode")) {
      g_object_set (element, "thumbnail-mode", playbin->thumbnail_mode, NULL);
    }
    ...
    // 2.5 ...and try to link
#ifdef INCLUDE_LMF
    select->srcpad = lgsbin_srcpad;
#endif
    if ((gst_pad_link (select->srcpad, sinkpad)) != GST_PAD_LINK_OK) {
      GST_WARNING_OBJECT (playbin, "Link failed on pad %s:%s",
          GST_DEBUG_PAD_NAME (sinkpad));
      gst_element_set_state (element, GST_STATE_NULL);
      gst_object_unref (sinkpad);
    ...
    GST_DEBUG_OBJECT (playbin, "linked on pad %s:%s",
        GST_DEBUG_PAD_NAME (select->srcpad));
    ...
    g_object_set_data (G_OBJECT (select->srcpad), "playbin2.sinkelement",
        element);
    gst_caps_unref (caps);
    ...
    elements =
        gst_element_factory_list_get_elements
        (GST_ELEMENT_FACTORY_TYPE_AUDIOVIDEO_SINKS, GST_RANK_PRIMARY);
    elements = g_list_sort (elements, gst_plugin_feature_rank_compare_func);
    ...
    GST_DEBUG_OBJECT (playbin, "found factories %" GST_PTR_FORMAT ": %d",
        mylist, g_list_length (mylist));

    GST_PLUGIN_FEATURE_LIST_DEBUG (mylist);


    /* Check if there is a proper sink */
    if (mylist == NULL) {
      gst_caps_unref (caps);
      GST_DEBUG_OBJECT (playbin, "No SINK for caps: %" GST_PTR_FORMAT, caps);
      gst_plugin_feature_list_free (elements);
      return NULL;
    }
    ...
    for (tmp = mylist; tmp; tmp = g_list_next (tmp)) {
      factory = GST_ELEMENT_FACTORY_CAST (tmp->data);
      GST_DEBUG_OBJECT (playbin, "Element list : name : %s",
          gst_plugin_feature_get_name (GST_PLUGIN_FEATURE (factory)));
      ...
      switch (select->type) {
        case GST_PLAY_SINK_TYPE_AUDIO:
        case GST_PLAY_SINK_TYPE_AUDIO_RAW:
          playbin->audio_sink = element;
          GST_DEBUG_OBJECT (playbin, "sink type : Audio, %" GST_PTR_FORMAT,
              playbin->audio_sink);
          break;
        case GST_PLAY_SINK_TYPE_VIDEO:
        case GST_PLAY_SINK_TYPE_VIDEO_RAW:
          playbin->video_sink = element;
          GST_DEBUG_OBJECT (playbin, "sink type : Video, %" GST_PTR_FORMAT,
              playbin->video_sink);
          break;
        case GST_PLAY_SINK_TYPE_TEXT:
          playbin->text_sink = element;
          GST_DEBUG_OBJECT (playbin, "sink type : Text, %" GST_PTR_FORMAT,
              playbin->text_sink);
          break;
        default:
          GST_DEBUG_OBJECT (playbin, "no supported type ");
      }
...
#ifdef INCLUDE_LMF
  capsstr = gst_caps_to_string (caps);
  GST_DEBUG_OBJECT (playbin, "caps : %s", capsstr);
  g_free (capsstr);
#endif
//2013-03-26 gstreamer upgrade end
...
//2013-03-26 gstreamer upgrade start	
	select->selector = gst_element_factory_make ("lgselectorbin", NULL);
#ifdef INCLUDE_LMF              /* for 3D daul stream */
	if (gst_structure_has_field (s, "typeof3D")) {
	  guint typeof3D = 0;

	  typeof3D = g_value_get_uint (gst_structure_get_value (s, "typeof3D"));
	  if (typeof3D == 0x0D)
		g_object_set (select->selector, "bypass", TRUE, NULL);
	}
	gboolean is_stream = FALSE;
	if (strlen (group->uri) > strlen("file")) {
	  is_stream = 
		  g_ascii_strncasecmp ("file", group->uri, strlen ("file")) != 0;
	}
	g_object_set (select->selector, "is-stream", is_stream, NULL);
#endif
//2013-03-26 gstreamer upgrade end
...
//2013-03-26 gstreamer upgrade start
      ...
      if (!it
          || (gst_iterator_next (it, (gpointer) & srcpad)) != GST_ITERATOR_OK
          || srcpad == NULL) {

        GST_ERROR_OBJECT (select->selector,
            "Couldn't get srcpad from selectorbin for sinkpad %" GST_PTR_FORMAT,
            sinkpad);

        goto src_link_failed;
      }

      if (select->padpairs == NULL)
        select->padpairs = g_hash_table_new (g_direct_hash, g_direct_equal);

      g_hash_table_insert (select->padpairs, sinkpad, srcpad);
//          gst_object_ref (sinkpad), gst_object_ref (srcpad));
      select->npadpairs++;
#endif
//2013-03-26 gstreamer upgrade end	  
    }
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
		GstPad *selpad; 			//input-selector's sinkpad of selectorbin
		selpad = gst_ghost_pad_get_target (GST_GHOST_PAD_CAST (sinkpad));
		switch (select->type) {
		  case GST_PLAY_SINK_TYPE_VIDEO:
		  case GST_PLAY_SINK_TYPE_VIDEO_RAW:
			...
			g_object_set (selpad, "always-ok", always_ok, NULL);
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
    if (g_hash_table_remove (select->padpairs, peer)) {
      if (--select->npadpairs == 0) {
//        g_hash_table_unref (select->padpairs);
        g_hash_table_destroy (select->padpairs);
        select->padpairs = NULL;
      }
      GST_INFO_OBJECT (playbin,
          "padpairs key/value removed, number of padpair = %d",
          select->npadpairs);
    }
#endif
//2013-03-26 gstreamer upgrade end
...
#ifdef INCLUDE_LMF
	if (select->npadpairs > 0 && select->sinkpad == NULL) {
	  GST_DEBUG_OBJECT (playbin, "requesting new sink pad %d", select->type);
	  g_object_get (select->selector, "bypass", &bypass, NULL);
	  if (bypass && i == PLAYBIN_STREAM_VIDEO) {
		for (j = 0; j < select->npadpairs; j++) {
		  GstPad *lgsbin_srcpad = NULL;
		  GList *temp = NULL;
		  temp = g_hash_table_get_values (select->padpairs);
		  lgsbin_srcpad = g_list_nth_data (temp, j);
		  //FIXME autoplug_sink return value handle
		  autoplug_sink (select, playbin, lgsbin_srcpad);
		  g_list_free (temp);
		}
	  } else {
		GstPad *lgsbin_srcpad = NULL;
		GList *temp = NULL;
		temp = g_hash_table_get_values (select->padpairs);
		lgsbin_srcpad = g_list_nth_data (temp, 0);
		select->sinkpad = autoplug_sink (select, playbin, lgsbin_srcpad);
		g_list_free (temp);
	  }
	  GST_DEBUG_OBJECT (playbin, "linked type %s", select->media_list[0]);
	}
#else
...
shutdown:
  {
    ...
    /* Request a flushing pad from playsink that we then link to the selector.
       ...
       */
#ifdef INCLUDE_LMF
     GST_DEBUG ("LMF ignores shutdown process");
#else
     GST_SOURCE_GROUP_LOCK (group);
     for (i = 0; i < PLAYBIN_STREAM_LAST; i++) {
       GstSourceSelect *select = &group->selector[i];
    ...
    GstCaps * caps, GstSourceGroup * group)
{
  ...
  g_mutex_unlock (&playbin->elements_lock);
//2013-03-26 gstreamer upgrade start
  GST_DEBUG_OBJECT (playbin, "found factories %d", g_list_length (mylist));
//2013-03-26 gstreamer upgrade end
  GST_PLUGIN_FEATURE_LIST_DEBUG (mylist);
...
//2013-03-26 gstreamer upgrade start  
#ifdef INCLUDE_LMF
  GST_DEBUG_OBJECT (playbin, "but LMF ignores process");
#else
...
#ifdef INCLUDE_LMF
static void
sig_mms_handler (GstElement * mms_src, gdouble rate, GstSourceGroup * group)
{
  GstPlayBin *playbin;
  playbin = group->playbin;

  GST_DEBUG_OBJECT (playbin, "mms trick play - rate : %f", rate);
  ...
  if (playbin->audio_sink) {
    ...
    GST_DEBUG_OBJECT (playbin, "set audiosink's trickplay to %d", bST);
    g_object_set (playbin->audio_sink, "serverside-trickplay", bST, NULL);
  } else
    GST_DEBUG_OBJECT (playbin, "no audiosink");
}
#endif
//2013-03-26 gstreamer upgrade end
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
	/*if the source is mms, register callback for serverside trickplay */
	if (!strcmp (GST_ELEMENT_NAME (playbin->source), "mmssrc")) {
	  g_signal_connect (playbin->source, "mms-playrate-changed",
		  G_CALLBACK (sig_mms_handler), group);
	}
  
	/*set content length if playbin2 received it from LMF  */
	if (group->content_length
		&& g_object_class_find_property (G_OBJECT_GET_CLASS (playbin->source),
			"dlna-contentlength")) {
	  GST_DEBUG_OBJECT (playbin, "dlna content length  : %" G_GUINT64_FORMAT,
		  group->content_length);
	  g_object_set (playbin->source, "dlna-contentlength", group->content_length,
		  NULL);
	}
  
	if (group->cleartext_size
		&& g_object_class_find_property (G_OBJECT_GET_CLASS (playbin->source),
			"dlna-cleartextsize")) {
	  GST_DEBUG_OBJECT (playbin, "dlna cleartext size  : %" G_GUINT64_FORMAT,
		  group->cleartext_size);
	  g_object_set (playbin->source, "dlna-cleartextsize", group->cleartext_size,
		  NULL);
	}
  
	if (group->opval != 0x111
		&& g_object_class_find_property (G_OBJECT_GET_CLASS (playbin->source),
			"dlna-opval")) {
	  GST_DEBUG_OBJECT (playbin, "dlna opval  : %" G_GUINT32_FORMAT,
		  group->opval);
	  g_object_set (playbin->source, "dlna-opval", group->opval, NULL);
	}
  
	if (group->flagval != 0x111
		&& g_object_class_find_property (G_OBJECT_GET_CLASS (playbin->source),
			"dlna-opval")) {
	  GST_DEBUG_OBJECT (playbin, "dlna flagval	: %" G_GUINT32_FORMAT,
		  group->flagval);
	  g_object_set (playbin->source, "dlna-flagval", group->flagval, NULL);
	}
#endif
//2013-03-26 gstreamer upgrade end
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
  if (group->current_title != -1)
    g_object_set (group->uridecodebin, "current-title", group->current_title,
	  NULL);
  
  if (group->title_offset != -1)
    g_object_set (group->uridecodebin, "title-offset", group->title_offset,
  	  NULL);
  
  if (group->protocol_info) {
	g_object_set (group->uridecodebin, "dlna-protocolinfo",
	  group->protocol_info, NULL);
	GST_DEBUG_OBJECT (playbin, "set uridecodebin's protocolfinfo : %s",
	  group->protocol_info);
  } else {
	GST_DEBUG_OBJECT (playbin, "no protocolinfo %p", group);
  }
  
  if (group->dlna_duration) {
	g_object_set (group->uridecodebin, "dlna-duration",
	  group->dlna_duration, NULL);
	GST_DEBUG_OBJECT (playbin, "set uridecodebin's dlna-duration : %s",
	  group->dlna_duration);
  } else {
	GST_DEBUG_OBJECT (playbin, "no dlna-duration value %p", group);
  }
  
  g_object_set (group->uridecodebin, "dlna-cleartextsize",
	group->cleartext_size, NULL);
  GST_DEBUG_OBJECT (playbin, "set uridecodebin's dlna:cleartextSize : %d",
	group->cleartext_size);
  
  // Setting thumbnail mode on uridecodebin. (hozi.kim / 2012.07.02)
  if (g_object_class_find_property (G_OBJECT_GET_CLASS (uridecodebin),
	"thumbnail-mode"))
  g_object_set (uridecodebin, "thumbnail-mode", playbin->thumbnail_mode,
    NULL);
  
  if (g_object_class_find_property (G_OBJECT_GET_CLASS (uridecodebin),
	"playbin-mode"))
    g_object_set (uridecodebin, "playbin-mode", TRUE, NULL);
  
  
  if (group->uridecodebin) {
	if (g_object_class_find_property (G_OBJECT_GET_CLASS (uridecodebin),
	  "dlna-opval"))
	  g_object_set (group->uridecodebin, "dlna-opval", group->opval, NULL);
  
	if (g_object_class_find_property (G_OBJECT_GET_CLASS (uridecodebin),
	  "dlna-protocolinfo"))
	  g_object_set (uridecodebin, "dlna-protocolinfo", group->protocol_info,
			NULL);
	if (g_object_class_find_property (G_OBJECT_GET_CLASS (uridecodebin),
	  "vdec-channelreserved"))
      g_object_set (uridecodebin, "vdec-channelreserved",
	  group->vdec_channelreserved, NULL);
  }
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
  if (playbin->drm_type != DEFAULT_DRM_TYPE)
	g_object_set (uridecodebin, "drm-type", playbin->drm_type, NULL);
#endif
  //2013-03-26 gstreamer upgrade end
...
//2013-03-26 gstreamer upgrade start		
#ifdef INCLUDE_LMF
	/* detach sink & unlink and delete it */
	{
	  GstElement *pSinkElement = NULL;
	  GST_DEBUG_OBJECT (playbin, " Delete sink elements");

	  pSinkElement =
		  g_object_get_data (G_OBJECT (select->srcpad),
		  "playbin2.sinkelement");
	  if (pSinkElement) {
		GST_DEBUG_OBJECT (playbin, " sink element name: %s",
			GST_ELEMENT_NAME (pSinkElement));
		...
		if (group->audio_sink) {
		  ...
		  gst_object_unref (group->audio_sink);
		  GST_INFO_OBJECT(playbin, "\n++++ group->audio_sink->ref_count = [%d]++++\n",
			  G_OBJECT(group->audio_sink)->ref_count);
		  GST_INFO_OBJECT(playbin, "\n++++ [audio_sink] group=[%p] playbin=[%p]++++\n",
			group->audio_sink, playbin->audio_sink );
		  group->audio_sink = NULL;
		}
		if (group->video_sink) {
		  /* If this is a automatically created sink set it to NULL */
		  ...
		  gst_object_unref (group->video_sink);
		  GST_INFO_OBJECT(playbin, "\n++++ group->video_sink->ref_count = [%d]++++\n",
				  G_OBJECT(group->video_sink)->ref_count);
		  GST_INFO_OBJECT(playbin, "\n++++ [video_sink] group=[0x%p] playbin=[0x%p]++++\n",
			group->video_sink, playbin->video_sink );
		  group->video_sink = NULL;
		}

		gst_object_ref (pSinkElement);
		//gst_element_set_state (pSinkElement, GST_STATE_NULL);
		gst_bin_remove (GST_BIN_CAST (playbin), pSinkElement);
		GST_INFO_OBJECT(playbin, "\n++++[%d] [%p] pSinkElement->ref_count = [%d]++++\n", select->type, pSinkElement, G_OBJECT(pSinkElement)->ref_count);

		if( G_OBJECT(pSinkElement)->ref_count == 0 )
			pSinkElement = NULL;

		g_object_set_data (G_OBJECT (select->srcpad),
			"playbin2.sinkelement", NULL);

		switch (select->type) {
		  case GST_PLAY_SINK_TYPE_AUDIO:
		  case GST_PLAY_SINK_TYPE_AUDIO_RAW:
			playbin->audio_sink = pSinkElement;
			GST_DEBUG_OBJECT (playbin, "audio_sink = null");
			break;
		  case GST_PLAY_SINK_TYPE_VIDEO:
		  case GST_PLAY_SINK_TYPE_VIDEO_RAW:
			playbin->video_sink = pSinkElement;
			GST_DEBUG_OBJECT (playbin, "video_sink	= null");
			break;
		  case GST_PLAY_SINK_TYPE_TEXT:
			playbin->text_sink = pSinkElement;
			GST_DEBUG_OBJECT (playbin, "text_sink = null");
			break;
		  default:
			break;
		}
	  } else {
		GST_DEBUG_OBJECT (playbin, " no sink to be deleted");
	  }
	}
#else
	gst_play_sink_release_pad (playbin->playsink, select->sinkpad);
...
do_async_start (GstPlayBin * playbin)
{
  ...
  GST_INFO_OBJECT (playbin, "Sending async_start message");
...
do_async_done (GstPlayBin * playbin)
{
  ...
  if (playbin->async_pending) {
    GST_INFO_OBJECT (playbin, "Sending async_done message");
    message = gst_message_new_async_done (GST_OBJECT_CAST (playbin), FALSE);
    GST_BIN_CLASS (parent_class)->handle_message (GST_BIN_CAST
        (playbin), message);
 ...
 */
static gboolean
gst_play_bin_send_event_to_sink (GstPlayBin * playbin, GstEvent * event)
{
  gboolean res = TRUE;

  if (playbin->video_sink) {
    gst_event_ref (event);
    if ((res = gst_element_send_event (playbin->video_sink, event))) {
      GST_DEBUG_OBJECT (playbin, "Sent event succesfully to video sink");
      goto done;
    }
    GST_DEBUG_OBJECT (playbin, "Event failed when sent to video sink");
  }
  if (playbin->audio_sink) {
    gst_event_ref (event);
    if ((res = gst_element_send_event (playbin->audio_sink, event))) {
      GST_DEBUG_OBJECT (playbin, "Sent event succesfully to audio sink");
      goto done;
    }
    GST_DEBUG_OBJECT (playbin, "Event failed when sent to audio sink");
  }
...
//2013-03-26 gstreamer upgrade start
      ...
      bret = GST_STATE_CHANGE_ASYNC;
#endif
//2013-03-26 gstreamer upgrade end	  
      break;
    case GST_STATE_CHANGE_PAUSED_TO_READY:
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF              // for prevent audio-only lock-up case.  geunil.jung
      if (!playbin->thumbnail_mode && !n_video) {
        GST_PLAY_BIN_LOCK (playbin);
        usleep (100 * 1000);
        GST_PLAY_BIN_UNLOCK (playbin);
      }
...
//2013-03-26 gstreamer upgrade start
#ifdef INCLUDE_LMF
  if (ret != GST_STATE_CHANGE_NO_PREROLL && bret == GST_STATE_CHANGE_ASYNC)
    ret = bret;
#endif
//2013-03-26 gstreamer upgrade end
