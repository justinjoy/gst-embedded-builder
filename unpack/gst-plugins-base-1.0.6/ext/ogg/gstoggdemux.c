...
/* 2012. 10. 18. geunil.jung.
 * NETCAST40_DQMS_NC40_INIT-11217. 1210-01798
 * play/seek/playtime play/multi-track switch on video with vorbis audio,
 * there is no sound.
 * mkv demux와 같은 방법으로 수정.
 */
#define ENABLE_VORBIS_PATCH_OGG
...
//2013-03-27 gstreamer upgrade end
  ...
  //2013-03-27 gstreamer upgrade start
#ifdef ENABLE_VORBIS_PATCH_OGG
	if( !strstr(name, "vorbis") ) { // not a vorbis
	  while (headers) {
		GValue value = { 0 };
		GstBuffer *buffer;
	  ...
	  else
	  {
		if (buf) {
		GstBuffer * buf_tmp;
		buf_tmp = gst_buffer_copy (buf);
		//buf_tmp = gst_buffer_new_and_alloc (total_len);
		gst_buffer_unref (buf);
		buf = gst_buffer_new_and_alloc (total_len);

		if (total_len - op->bytes)
		  gst_buffer_fill (buf, 0, buf_tmp, total_len - op->bytes);

		if (op->bytes)
	      gst_buffer_fill (buf, total_len - op->bytes, op->packet, op->bytes);

		gst_buffer_unref (buf_tmp);
		}
	  }
	  headers = headers->next;
	  }
...
//2013-03-27 gstreamer upgrade start
#ifdef ENABLE_VORBIS_PATCH_OGG
    st = gst_caps_get_structure(pad->map.caps, 0);
    mime = gst_structure_get_name(st);
  
    if (!strstr (mime, "vorbis") )
    {
